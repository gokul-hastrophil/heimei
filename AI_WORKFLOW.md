...
`ai_require_branch_protection` (in `Scripts/ai/common.sh`, called unconditionally by `dispatch.sh` — identically in dry-run and execute) dies, never warns, when: the GitHub API call fails for any reason (network, auth, 403, 404 — all indistinguishable in effect, none of them proves protection exists); the response is missing a required field or isn't valid JSON; pull-request enforcement (`required_pull_request_reviews` present with a non-negative approval count; Heimei's solo-maintainer configuration uses 0), force-push prevention, deletion prevention, or required status checks can't be confirmed; `enforce_admins` isn't `true`; or any active repository ruleset declares a bypass actor (checked conservatively, repository-wide, not scoped to rulesets provably targeting this one branch). **This does not prove the specific credential `gh` is authenticated as cannot bypass protection** — GitHub does not expose "is actor X exempt from rule Y" as a simple boolean for classic branch protection; `enforce_admins=true` plus "no ruleset declares any bypass actor at all" is the closest mechanical confirmation available, not a stronger claim than that.
...
The canonical-ADR lookup distinguishes an ADR that already existed at the approval's base SHA (extracted from that immutable base, exactly as above — never weakened) from one the very PR under review itself introduces (necessarily absent at base by construction): the latter is resolved only from the PR's own already-validated, locally-fetched changed-file manifest, checked against the approval's allowed paths and the policy denylist, then extracted from the exact reviewed PR-head SHA and labeled in the packet as newly proposed, under review — never as pre-existing accepted background truth. A reference that matches neither case, including the architecture issue template's own `New ADR needed` placeholder with no ADR number attached, is handled without inventing a false match. The lookup extracts every DISTINCT `ADR-####` identifier the field names, not merely the first — repeated mentions of the same identifier resolve deterministically, but two or more genuinely different identifiers (for example prose explaining that one number is already reserved and a different one was chosen instead) fail closed as ambiguous rather than silently picking whichever matched first. See issue #14 for the full defect this closes (every new-ADR PR was previously unreviewable) and its regression tests.
